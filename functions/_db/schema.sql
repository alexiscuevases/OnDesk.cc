...
-- ─── Marketplace & Tools ──────────────────────────────────────────────────
--
-- A "product" is a connector definition: any REST API described endpoint by
-- endpoint (Calendly, Stripe, an internal enterprise API…).
--   · workspace_id IS NULL + is_public = 1  → catalog template shipped by Pulse
--   · workspace_id = <ws>                   → private connector built by that
--     workspace (visible only to it unless is_public is set by an admin)
--
-- Credentials NEVER live on the product — only on the install
-- (workspace_products.credentials, AES-GCM encrypted at rest).
...
CREATE TABLE IF NOT EXISTS products (
  ...
  description     TEXT,
  logo_url        TEXT,
  -- 'scheduling' | 'payments' | 'crm' | 'ecommerce' | 'communication' | 'internal' | 'other'
  category        TEXT    NOT NULL DEFAULT 'other',
  docs_url        TEXT,
  -- Root of the API, e.g. https://api.stripe.com. May contain {{config_key}}
  -- placeholders resolved from the install's settings (per-tenant hosts).
  base_url        TEXT    NOT NULL DEFAULT '',
  -- 'none' | 'bearer' | 'api_key_header' | 'api_key_query' | 'basic' | 'custom'
  auth_type       TEXT    NOT NULL DEFAULT 'none',
  auth_config     TEXT    NOT NULL DEFAULT '{"type":"none"}', -- JSON AuthConfig
  -- JSON [{ key, label, type, secret, required, placeholder, help }] — what the
  -- installing workspace must fill in (tokens, account ids, tenant host, …)
  config_fields   TEXT    NOT NULL DEFAULT '[]',
  default_headers TEXT    NOT NULL DEFAULT '{}',  -- JSON, sent on every request
  is_public       INTEGER NOT NULL DEFAULT 0,
  created_by      TEXT    REFERENCES users(id) ON DELETE SET NULL, -- NULL = system template
  created_at      INTEGER NOT NULL DEFAULT (unixepoch()),
  updated_at      INTEGER NOT NULL DEFAULT (unixepoch())
);
...
-- Product Actions: one row per registered endpoint. This is what an AI agent
-- can call — each row becomes one tool in the agent's prompt.
CREATE TABLE IF NOT EXISTS product_actions (
  id                    TEXT    PRIMARY KEY DEFAULT (lower(hex(randomblob(16)))),
  product_id            TEXT    NOT NULL REFERENCES products(id) ON DELETE CASCADE,
  name                  TEXT    NOT NULL, -- snake_case identifier, e.g. find_customer_by_email
  description           TEXT    NOT NULL, -- the AI reads this to decide when to call it
  method                TEXT    NOT NULL DEFAULT 'GET', -- GET|POST|PUT|PATCH|DELETE
  path                  TEXT    NOT NULL DEFAULT '/',   -- appended to base_url; {param} = path param
  content_type          TEXT    NOT NULL DEFAULT 'json', -- 'json' | 'form' | 'none'
  -- JSON [{ name, in: 'path'|'query'|'body'|'header', type, required, description, enum, default }]
  parameters            TEXT    NOT NULL DEFAULT '[]',
  headers               TEXT    NOT NULL DEFAULT '{}',  -- JSON, action-specific headers
  response_path         TEXT,   -- dot path to project from the response, e.g. "data"
  requires_confirmation INTEGER NOT NULL DEFAULT 0, -- write ops: agent must escalate to a human
...
CREATE TABLE IF NOT EXISTS workspace_products (
  ...
  workspace_id    TEXT    NOT NULL REFERENCES workspaces(id) ON DELETE CASCADE,
  product_id      TEXT    NOT NULL REFERENCES products(id) ON DELETE CASCADE,
  credentials     TEXT,   -- AES-GCM encrypted JSON of the secret config fields
  settings        TEXT    NOT NULL DEFAULT '{}', -- JSON of the non-secret config fields
...
-- allowed_actions = JSON array of action names; NULL/[] means "every action".
...
-- Tool Call Logs: audit trail of every outbound API call made through a
-- connector, whether triggered by an AI agent or run manually from the console.
CREATE TABLE IF NOT EXISTS tool_call_logs (
  ...
  action_id            TEXT    NOT NULL, -- the prefixed id the agent used
  method               TEXT,
  url                  TEXT,             -- secrets stripped
  request_params       TEXT,             -- JSON, secrets stripped
  status_code          INTEGER,
