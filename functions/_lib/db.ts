import type {
  ...
  AiAgentRow, PublicAiAgent, AiAgentStatus, AiAgentAction,
  AiAgentMailboxRow, PublicAiAgentMailbox,
  AiTicketStateRow, PublicAiTicketState,
  AiActionLogRow, PublicAiActionLog,
  ProductRow, WorkspaceProductRow, AgentToolRow, PublicProduct, PublicWorkspaceProduct
} from "./types";
...
export async function findAiAgentsByWorkspace(
  db: D1Database,
  workspaceId: string
): Promise<PublicAiAgent[]> {
  const result = await db
    .prepare("SELECT * FROM ai_agents WHERE workspace_id = ? ORDER BY created_at DESC")
    .bind(workspaceId)
    .all<AiAgentRow>();
...
export async function findAiAgentById(
  ...
  const result = await db
    .prepare("SELECT * FROM ai_agents WHERE id = ? LIMIT 1")
    .bind(agentId)
    .first<AiAgentRow>();
  return result ?? null;
}

export async function findActiveAgentForMailbox(
  db: D1Database,
  mailboxIntegrationId: string
): Promise<AiAgentRow | null> {
  const result = await db
...
export async function updateAiAgent(
  ...
  if (data.confidence_threshold !== undefined){ fields.push("confidence_threshold = ?"); values.push(data.confidence_threshold); }
  if (data.max_auto_replies !== undefined)    { fields.push("max_auto_replies = ?");     values.push(data.max_auto_replies); }
  if (fields.length === 0) return;
  fields.push("updated_at = unixepoch()");
  values.push(agentId);
  await db
...
export async function findMailboxesForAgent(
  db: D1Database,
  agentId: string
): Promise<PublicAiAgentMailbox[]> {
  const result = await db
    .prepare(
...
export async function assignMailboxToAgent(
  db: D1Database,
  agentId: string,
  mailboxIntegrationId: string
): Promise<void> {
  ...
  await db
    .prepare(
      "INSERT OR IGNORE INTO ai_agent_mailboxes (id, ai_agent_id, mailbox_integration_id) VALUES (?, ?, ?)"
    )
    .bind(id, agentId, mailboxIntegrationId)
    .run();
}

export async function unassignMailboxFromAgent(
  db: D1Database,
  agentId: string,
  mailboxIntegrationId: string
): Promise<void> {
  await db
    .prepare("DELETE FROM ai_agent_mailboxes WHERE ai_agent_id = ? AND mailbox_integration_id = ?")
    .bind(agentId, mailboxIntegrationId)
    .run();
}

export async function setAgentMailboxEnabled(
  db: D1Database,
  agentId: string,
  mailboxIntegrationId: string,
  enabled: boolean
  ...
  await db
    .prepare(
      "UPDATE ai_agent_mailboxes SET enabled = ? WHERE ai_agent_id = ? AND mailbox_integration_id = ?"
    )
...
export async function findAiTicketState(
  db: D1Database,
  ticketId: string
): Promise<PublicAiTicketState | null> {
  const result = await db
    .prepare("SELECT * FROM ai_ticket_state WHERE ticket_id = ? LIMIT 1")
    .bind(ticketId)
    .first<AiTicketStateRow>();
  return result ? rowToPublicAiTicketState(result) : null;
}

export async function createAiTicketState(
  db: D1Database,
  ticketId: string,
  agentId: string
): Promise<PublicAiTicketState> {
  const id = crypto.randomUUID();
  await db
    .prepare(
      "INSERT INTO ai_ticket_state (id, ticket_id, ai_agent_id) VALUES (?, ?, ?)"
    )
    .bind(id, ticketId, agentId)
    .run();
  const row = await db
    .prepare("SELECT * FROM ai_ticket_state WHERE id = ? LIMIT 1")
    .bind(id)
    .first<AiTicketStateRow>();
  return rowToPublicAiTicketState(row!);
}

export async function incrementAiReplyCount(
  db: D1Database,
  ticketId: string
): Promise<void> {
  await db
    .prepare(
      "UPDATE ai_ticket_state SET reply_count = reply_count + 1, updated_at = unixepoch() WHERE ticket_id = ?"
    )
    .bind(ticketId)
    .run();
}

export async function escalateAiTicket(
  db: D1Database,
  ticketId: string,
  note: string
): Promise<void> {
  await db
    .prepare(
      "UPDATE ai_ticket_state SET escalated = 1, escalated_at = unixepoch(), escalation_note = ?, updated_at = unixepoch() WHERE ticket_id = ?"
    )
    .bind(note, ticketId)
    .run();
}

export async function resumeAiTicket(
  db: D1Database,
  ticketId: string
): Promise<void> {
  await db
    .prepare(
      "UPDATE ai_ticket_state SET escalated = 0, escalated_at = NULL, escalation_note = NULL, updated_at = unixepoch() WHERE ticket_id = ?"
    )
    .bind(ticketId)
    .run();
}
...
export async function createAiActionLog(
  ...
  await db
    .prepare(
      "INSERT INTO ai_action_logs (id, ticket_id, ai_agent_id, action, metadata) VALUES (?, ?, ?, ?, ?)"
    )
    .bind(
      id,
      data.ticket_id,
...
export async function findAiActionLogsByTicket(
  db: D1Database,
  ticketId: string
): Promise<PublicAiActionLog[]> {
  const result = await db
    .prepare("SELECT * FROM ai_action_logs WHERE ticket_id = ? ORDER BY created_at DESC")
    .bind(ticketId)
    .all<AiActionLogRow>();
...
export async function findAiActionLogsByAgent(
  db: D1Database,
  agentId: string,
  ticketId?: string
): Promise<PublicAiActionLog[]> {
...
export async function findAllPublicProducts(db: D1Database): Promise<PublicProduct[]> {
  const result = await db
    .prepare("SELECT * FROM products WHERE is_public = 1 ORDER BY name ASC")
    .all<ProductRow>();
  return (result.results ?? []).map((r) => ({
    id: r.id,
    name: r.name,
...
export async function updateWorkspaceProductConfig(
  db: D1Database,
...
export async function assignToolToAgent(db: D1Database, agentId: string, workspaceProductId: string): Promise<void> {
  const id = crypto.randomUUID();
  await db
    .prepare("INSERT OR IGNORE INTO agent_tools (id, ai_agent_id, workspace_product_id) VALUES (?, ?, ?)")
    .bind(id, agentId, workspaceProductId)
    .run();
}
