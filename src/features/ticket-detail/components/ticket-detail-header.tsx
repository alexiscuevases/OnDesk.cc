...
export function TicketDetailHeader({ ticket, onBack, onEditTitle, onDelete }: TicketDetailHeaderProps) {
	return (
		<div className="flex items-start gap-4">
			<Button variant="ghost" size="icon" onClick={onBack} className="size-9 shrink-0 mt-0.5">
				<ArrowLeft className="size-4" />
				<span className="sr-only">Back to tickets</span>
			</Button>
			<div className="flex-1 min-w-0">
				<div className="flex items-center gap-2 mb-1">
					<span className="text-xs font-mono font-semibold text-primary/70 dark:text-accent/70"><span className="text-muted-foreground">#</span>{ticket.number}</span>
					<StatusBadge status={ticket.status} showIcon size="md" />
				</div>
				<button
					onClick={onEditTitle}
					className="group text-xl font-black tracking-tight text-balance text-left hover:text-primary transition-colors">
					{ticket.subject}
